...
2. 难度循序渐进：根据用户当前水平选择起点，确保每周平均难度不低于前一周
...
内容优先级规则：
8. priority=supplementary的资源（Python入门、基础数学、传统ML入门）仅推荐给完全零基础用户；有编程经验的用户应跳过这些资源，直接从AI核心内容（LLM、Agent、RAG等）开始
9. 对非零基础用户，路径应以AI时代核心技能为主线：LLM原理→Prompt工程→RAG/Agent→部署实战
10. 基础补充（如果需要）以最小必要量穿插，不超过总路径的15%

教学质量规则：
11. 每周的资源要构成逻辑闭环：概念学习→动手实践→反思巩固
12. 关键概念（如 Transformer、RAG、Agent 架构）安排两次接触：首次粗学+后续深入
13. 如果提供了推荐大牛（builders），在路径相关阶段推荐关注，用 builders 字段引用 id
14. tip 要具体可执行，例如"用 LangChain 实现一个 RAG demo"而非"多练习"
15. 输出纯JSON
