...
### 必读文件
每次开始工作前，必须先读取以下文件：
- `docs/EVOLUTION.md` — 进化宪法，定义不可变原则和变更分级
- `docs/CONTENT_QUALITY.md` — 内容质量管理策略（涉及 resources.yaml 时必读）
...
**权威定义见 `docs/EVOLUTION.md` § 变更安全分级**。以下为快速参考的文件映射：

| 变更涉及的文件/区域 | 分级 |
|---------------------|------|
| `*.md`、CSS、测试文件、i18n 翻译 | 🟢 Safe |
| `views/*.py`（UI 逻辑）、`utils.py`、`resources.yaml` | 🟡 Moderate |
| `config.py` 中的 Prompt、`llm.py` 的调用参数、`docs/EVOLUTION.md` | 🔴 Dangerous |
...
- [ ] 读了 `docs/EVOLUTION.md` 且遵守所有不可变原则
...
2. 用户是否对 AGENTS.md / docs/EVOLUTION.md 做了修改 → 理解并遵守新规则
3. 是否有重复出现的同类问题 → 提取模式、加入防御规则

### 即时经验固化原则

**AI 犯错时，必须在修复的同一次交互中完成经验固化**，不得等待用户提醒。

流程：
1. **发现错误** → 修复技术问题
2. **提取教训** → 问自己"这个错误的根因是什么？如何从规则层面防止再犯？"
3. **固化为策略** → 写入 AGENTS.md 对应章节（反馈信号表 / 约束清单 / 安全规范等）
4. **一次提交** → 技术修复 + 策略固化在同一个 commit 中

> **教训来源**：AI 发现 E2E 测试产生真实 Issues 后修复了技术问题，但忘记将经验教训固化到 AGENTS.md，直到用户提醒才补上。"忘记固化" 本身就是需要固化的教训。
...
| `i18n.py` | 国际化翻译字典 |
| `resources.yaml` | 资源数据库（128条） |
| `views/` | 各页面渲染（form/path/chat/radar/browser/feedback/settings/progress/import_plan） |
| `tests/` | 单元测试 + E2E 测试 |
| `docs/EVOLUTION.md` | 进化宪法（不可变原则 + 分级规则） |
| `docs/CONTENT_QUALITY.md` | 内容质量管理策略（六维度 + 审计规范） |
| `docs/VISION.md` | 产品愿景（信念、用户画像、设计原则、成功指标） |
